...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 flex flex-col items-center justify-center p-4 gap-4 md:py-12">
        ...
        <div className="p-8 md:p-10 flex-grow">
            ...
            <>
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-10 block w-full border-gray-200 rounded-xl border p-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all placeholder-gray-400"
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full flex justify-center items-center py-3.5 px-4 border border-transparent rounded-xl shadow-lg shadow-indigo-600/10 text-xs font-bold uppercase tracking-wider text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  {isLoading ? (
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-10 block w-full border-gray-200 rounded-xl border p-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all placeholder-gray-400 font-mono"
          ...
          {step === 'password' && (
            <>
              <div className="text-center mb-6">
                ...
                <div className="mt-3 inline-block bg-slate-100 px-3 py-1 rounded-full text-[11px] text-gray-600 font-mono">
                  Logado como: <span className="font-semibold text-indigo-600">{email}</span>
                </div>
              </div>
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4">
                <div>
                  ...
                  {showForgotHelp && (
                    <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl text-xs text-indigo-900 leading-relaxed shadow-sm mt-3 flex flex-col gap-2 animate-fade-in text-left">
                      <div className="flex gap-2 font-bold text-indigo-950 items-center">
                      ...
                      <p>
                        ...
                        <a 
                          ...
                          className="font-bold text-indigo-700 hover:underline block mt-1"
                        >
                      ...
                      <button
                        ...
                        className="text-[10px] uppercase font-bold text-indigo-700 hover:text-indigo-900 self-end mt-1 border border-indigo-200 px-2.5 py-1 rounded-lg bg-white shadow-sm"
                      >
                        Entendi
                      </button>
                    </div>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 flex flex-col items-center gap-2">
            <button
              ...
              className="text-[10px] text-indigo-500 hover:text-indigo-700 hover:underline font-bold uppercase tracking-wider flex items-center justify-center gap-1 bg-indigo-50/50 hover:bg-indigo-50 px-3.5 py-2 rounded-xl transition-all border border-indigo-150 w-full animate-pulse-slow"
            >
        ...
        <div className="fixed inset-0 z-[10000] bg-black/70 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-gray-100 flex flex-col text-gray-800 animate-in zoom-in-95 duration-200 my-8">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className="text-[10px] font-black tracking-widest text-indigo-600 uppercase">Tecnologia Híbrida</span>
                <h3 className="text-xl font-black text-indigo-950 mt-0.5">Métodos Alternativos de Uso</h3>
              </div>
              <button 
                type="button"
            ...
            <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
              {/* Windows / Chrome / Edge */}
              <div className={`border rounded-xl p-4 flex gap-3 text-left transition-all ${detectedOS === 'windows' ? 'bg-indigo-50/50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-gray-50/50 border-gray-100'}`}>
                <div className={`p-2.5 h-fit rounded-lg shrink-0 ${detectedOS === 'windows' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
                  <Monitor className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-sm font-bold text-indigo-950">Navegadores de Computador (Chrome / Edge)</h4>
                    {detectedOS === 'windows' && <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-black uppercase">Seu Sistema</span>}
                  </div>
              ...
              <div className={`border rounded-xl p-4 flex gap-3 text-left transition-all ${detectedOS === 'mac' ? 'bg-indigo-50/50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-gray-50/50 border-gray-100'}`}>
                <div className={`p-2.5 h-fit rounded-lg shrink-0 ${detectedOS === 'mac' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
                  <Monitor className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-sm font-bold text-indigo-950">macOS (Apple Safari)</h4>
                    {detectedOS === 'mac' && <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-black uppercase">Seu Sistema</span>}
                  </div>
                  <p className="text-xs text-gray-600 leading-relaxed mb-2 mt-0.5">
              ...
              {/* iOS / iPhone */}
              <div className={`border rounded-xl p-4 flex gap-3 text-left transition-all ${detectedOS === 'ios' ? 'bg-indigo-50/50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-gray-50/50 border-gray-100'}`}>
                <div className={`p-2.5 h-fit rounded-lg shrink-0 ${detectedOS === 'ios' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
                  <Smartphone className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-sm font-bold text-indigo-950">iPhone / iPad (Apple iOS)</h4>
                    {detectedOS === 'ios' && <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-black uppercase">Seu Sistema</span>}
                  </div>
                  <p className="text-xs text-gray-600 leading-relaxed mb-2 mt-0.5">
            ...
            <div className="mt-5 flex justify-end">
              <button 
                type="button"
                onClick={() => setShowPwaGuide(false)}
                className="w-full px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs uppercase tracking-widest rounded-xl shadow-lg hover:shadow-indigo-500/20 transition-all text-center"
              >
                Entendi, fechar guia
              </button>
            </div>
